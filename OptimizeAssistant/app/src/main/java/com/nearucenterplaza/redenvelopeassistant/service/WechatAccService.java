...
public class WechatAccService extends AccessibilityService {
    ...
    public void onAccessibilityEvent(AccessibilityEvent event) {
        ...
        // TODO delete
        Log.d("wangchong", "currentActivityName = " + currentActivityName);
...
//    private static int mLastViewHashCode = 0;
    private static HashSet<Integer> openViewHashCode = new HashSet<>();
    ...
    public void handleChatOnly(AccessibilityNodeInfo node) {
        ...
        // TODO 需要考虑版本兼容
        List<AccessibilityNodeInfo> luckList = RedEnvelopeHelper.getLastWechatRedEnvelopeNodeListById(node);

        if (luckList != null && luckList.size() > 0) {
            Log.d("wangchong", "opened=== " + openViewHashCode.toString());

            // TODO delete
            StringBuffer sb = new StringBuffer();
            for (int i = 0; i < luckList.size(); i++) {
                sb.append(luckList.get(i).hashCode());
                sb.append(" ");
            }
            Log.d("wangchong", "HASHCODE: " + sb.toString());

            for (AccessibilityNodeInfo info : luckList) {
                if (openViewHashCode.contains(info.hashCode())) {
                    continue;
                } else {
                    openViewHashCode.add(info.hashCode());
...
//        if (tempNode != null && !openViewHashCode.contains(tempNode.hashCode())) {
//            // 不重复打开红包
//            mLastViewHashCode = tempNode.hashCode();
//            openViewHashCode.add(tempNode.hashCode());
